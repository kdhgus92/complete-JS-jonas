...
btn.addEventListener('click', () => {
  ...
  for (const [i, row] of rows.entries()) {
    const splitTextArr = row.trim().split('_');
    let camelCase = '';
    for (const word of splitTextArr) {
      camelCase += word[0].toUpperCase() + word.slice(1).toLowerCase();
    }
    camelCase = camelCase.replace(camelCase[0], camelCase[0].toLowerCase());
    camelCase = camelCase.padEnd(20, ' ');
    // count++;
    // console.log(camelCase + '✅'.repeat(count));
...
console.log(maskCreditCard(1234123));
console.log(maskCreditCard(4337782349239234));
console.log(maskCreditCard('32341523523452345'));

// Repeat
const message2 = 'Bad waether ...All Departuers Delayed... ';
...
console.log(airline.slice(4)); //새 문자열 반환
...
const question = new Map([
  ['question', 'What is the vest programming language inther world?'],
  [1, 'C'],
  [2, 'Java'],
  [3, 'JavaScript'],
  ['correct', 3],
...
let tot = 0;
for (const value of odds) tot += value;
const avgOdds = tot / odds.length;
console.log(avgOdds);
...
// always be the last in the destructuring assignment
// and not include skip element: " pasta "
...
const newMenu = [...restaurant.mainMenu, 'Gnocci']; //새로운 arr
console.log(newMenu);

// Copy array
const mainMenuCompy = [...restaurant.mainMenu];
...
// restaurantCopy.openingHours.fri.open = '223';
...
const [x, y, z] = arr; // destructuring arr
console.log(x, y, z);
console.log(arr);
